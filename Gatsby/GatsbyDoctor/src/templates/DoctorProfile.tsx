...
const DoctorProfile: React.FC<DoctorProfileProps> = ({ pageContext }) => {
  const doctor = mockDoctors.find(
    (d) => d.id === pageContext.doctorId
  ) as Doctor | null;
...
export const Head = ({ pageContext }: DoctorProfileProps) => {
  const doctor = mockDoctors.find((d) => d.id === pageContext.doctorId);
  return (
    <>
      <title>{doctor ? `${doctor.name} - ${doctor.specialty}` : 'Doctor Profile'} | DoctorBook</title>
      <meta name="description" content={doctor?.bio || 'Doctor profile'} />
    </>
  );
};
